...
public class PaperEnemy : Hazard
{
    ...
    // MUST BE TRUE: aggroRange > attackTimerStartDistance > aggroTargetStopDistance
    ...
    private bool gotToPatrolStartingPoint = false;
	...
	private void Start()
	{
        ...
        {
            Debug.LogError("THIS MUST BE TRUE: aggroRange > attackTimerStartDistance > aggroTargetStopDistance");
        }
    ...
    private Vector3 GetPatrolPositionForTime(float time)
    {
        ...
        return new Vector3(x, homePos.y, z);
    }
    ...
    private void AggroUpdate()
    {
        ...
        Vector3 targetPos = posAbovePlayer + ((transform.position - posAbovePlayer).normalized * aggroTargetStopDistance);
